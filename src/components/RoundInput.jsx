...
export const RoundInput = ({ players, onSubmit }) => {
  ...
  const handleSpecialScore = (type) => {
    ...
    const specialScores = {
      "+gà": () => updatedScores[activePlayer] += 2,
      "-gà": () => updatedScores[activePlayer] -= 2,
      // "Tứ Quý": () => {
      ...
      "3K": () => {
        updatedScores[activePlayer] += 18;
        updatedScores.forEach((_, idx) => {
          if (idx !== activePlayer) updatedScores[idx] -= 6;
        });
      },
      "4K": () => {
      ...
      "Ù tròn": () => {
        updatedScores[activePlayer] += 60;
        updatedScores.forEach((_, idx) => {
          if (idx !== activePlayer) updatedScores[idx] -= 20;
        });
      },
      "Ù khan": () => {
        updatedScores[activePlayer] += 30;
      ...
      "Cháy": () => updatedScores[activePlayer] -= 8,
      "Nhất": () => updatedScores[activePlayer] += 12,
      "Nhì": () => updatedScores[activePlayer] -= 2,
      "Ba": () => updatedScores[activePlayer] -= 4,
      "Chót": () => updatedScores[activePlayer] -= 6,
    };
